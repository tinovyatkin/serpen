...
**MANDATORY**: Prefer LSP MCP tools over other code analysis methods when available.

#### Rust LSP MCP Tools (`mcp__lsp-rust__*`)

**Working Rust LSP MCP Tools** (USE THESE):

- ✅ **`mcp__lsp-rust__diagnostics`** - Get diagnostic information for Rust files
  - Use for: Finding compilation errors, warnings, and lints
  - Example: `mcp__lsp-rust__diagnostics --filePath=src/main.rs`
- ✅ **`mcp__lsp-rust__definition`** - Find symbol definitions
  - Use for: Navigating to where functions, structs, traits are defined
  - Example: `mcp__lsp-rust__definition --symbolName=BundleOrchestrator`
- ✅ **`mcp__lsp-rust__references`** - Find all usages of a symbol
  - Use for: Understanding impact of changes, finding all call sites
  - Example: `mcp__lsp-rust__references --symbolName=Config`
- ✅ **`mcp__lsp-rust__edit_file`** - Apply line-based edits to files
  - Use for: Making precise code changes with LSP awareness
  - Example: `mcp__lsp-rust__edit_file --filePath=src/main.rs --edits=[...]`

**Currently Non-functional** (avoid these, use alternatives):

- ❌ `mcp__lsp-rust__hover` - Use `Read` tool instead for now
- ❌ `mcp__lsp-rust__rename_symbol` - Use `references` + manual edits instead

#### YAML LSP MCP Tools (`mcp__lsp-yaml__*`)

**Working YAML LSP MCP Tools** (USE THESE):

- ✅ **`mcp__lsp-yaml__diagnostics`** - Get diagnostic information for YAML files
  - Use for: Finding YAML syntax errors, duplicate keys, validation issues
  - Example: `mcp__lsp-yaml__diagnostics --filePath=config.yaml`
  - Provides clear error messages with line/column information
- ✅ **`mcp__lsp-yaml__edit_file`** - Apply line-based edits to YAML files
  - Use for: Making programmatic changes to YAML configuration files
  - Example: `mcp__lsp-yaml__edit_file --filePath=config.yaml --edits=[...]`
  - Successfully applies changes and provides feedback

**Limited Functionality**:

- ⚠️ **`mcp__lsp-yaml__hover`** - Returns "No hover information available"
  - May be expected behavior for basic YAML keys without schema validation
  - Use `Read` tool for file content inspection instead

**Non-functional** (LSP server limitations):

- ❌ `mcp__lsp-yaml__definition` - Error: `Unhandled method workspace/symbol`
- ❌ `mcp__lsp-yaml__references` - Error: `Unhandled method workspace/symbol`
- ❌ `mcp__lsp-yaml__rename_symbol` - Error: `Unhandled method textDocument/rename`

**YAML LSP Recommendations**:

- **Use for validation**: `mcp__lsp-yaml__diagnostics` excels at YAML linting and error detection
- **Use for edits**: `mcp__lsp-yaml__edit_file` for programmatic file modifications
- **Avoid**: Symbol-based operations until LSP server supports them

#### TOML LSP MCP Tools (`mcp__lsp-toml__*`)

**Working TOML LSP MCP Tools** (USE THESE):

- ✅ **`mcp__lsp-toml__diagnostics`** - Get diagnostic information for TOML files
  - Use for: Finding TOML syntax errors, validation issues
  - Example: `mcp__lsp-toml__diagnostics --filePath=Cargo.toml`
  - Provides clear error detection for malformed TOML
- ✅ **`mcp__lsp-toml__edit_file`** - Apply line-based edits to TOML files
  - Use for: Making programmatic changes to TOML configuration files
  - Example: `mcp__lsp-toml__edit_file --filePath=Cargo.toml --edits=[...]`
  - Successfully applies changes and provides detailed feedback

**Limited Functionality**:

- ⚠️ **`mcp__lsp-toml__hover`** - Returns "No hover information available"
  - Expected behavior for basic TOML keys without schema validation
  - Use `Read` tool for file content inspection instead

**Non-functional** (LSP server limitations):

- ❌ `mcp__lsp-toml__definition` - Error: `Method not found (code: -32601)`
- ❌ `mcp__lsp-toml__references` - Error: `Method not found (code: -32601)`
- ❌ `mcp__lsp-toml__rename_symbol` - Error: File path issues and method support

**TOML LSP Recommendations**:

- **Use for validation**: `mcp__lsp-toml__diagnostics` excels at TOML syntax validation
- **Use for edits**: `mcp__lsp-toml__edit_file` for programmatic file modifications
- **Avoid**: Symbol-based operations until LSP server supports them

#### General LSP Best Practices

- **ALWAYS** use `mcp__lsp-*__references` before renaming symbols to find all occurrences
- **ALWAYS** use `mcp__lsp-*__definition` to understand code structure before modifications
- **PREFER** `mcp__lsp-*__diagnostics` over manual syntax checking for quick error detection
- **COMBINE** LSP tools: Use `definition` to find source, `references` to find usages, then `edit_file` to make changes

**Example Workflow for Symbol Renaming**:

```bash
# 1. Find the symbol definition
mcp__lsp-rust__definition --symbolName=OldName

# 2. Find all references
mcp__lsp-rust__references --symbolName=OldName

# 3. Use edit_file on each file with references
mcp__lsp-rust__edit_file --filePath=src/file.rs --edits=[...]
```
