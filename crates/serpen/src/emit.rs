...
impl CodeEmitter {
    ...
    pub fn emit_bundle(&mut self, modules: &[&ModuleNode], entry_module: &str) -> Result<String> {
        ...
        for module in modules {
            ...
            let module_code =
                self.process_module_ast(&module.name, parsed_data, import_strategy)?;
        ...
        if let Some(entry_module_node) = modules.iter().find(|m| m.name == entry_module) {
            ...
            let entry_code = self.process_module_ast(
    ...
    /// Process a single module using AST transformations instead of string manipulation
    fn process_module_ast(
        ...
        // Transform AST: remove first-party imports and unused imports
        ...
        // Generate clean Python code using rustpython-unparser
        let mut unparser = Unparser::new();
        let module_code = self.unparse_module(&mut unparser, &transformed_ast)?;
