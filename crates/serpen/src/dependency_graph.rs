...
impl DependencyGraph {
    ...
    pub fn add_dependency(&mut self, from_module: &str, to_module: &str) -> Result<()> {
        let from_index = self.node_indices.get(from_module)
            .ok_or_else(|| anyhow!("Module not found: {}", from_module))?;
        let to_index = self.node_indices.get(to_module)
    ...
    pub fn topological_sort(&self) -> Result<Vec<&ModuleNode>> {
        let sorted_indices = toposort(&self.graph, None)
            .map_err(|cycle| anyhow!("Circular dependency detected involving module: {}", 
    ...
    pub fn get_dependencies(&self, module_name: &str) -> Option<Vec<&str>> {
        ...
        let dependencies: Vec<&str> = self.graph
    ...
    pub fn get_entry_modules(&self) -> Vec<&ModuleNode> {
        self.graph
            .node_indices()
            .filter(|&index| {
                self.graph.neighbors_directed(index, petgraph::Direction::Outgoing).count() == 0
            })
            .map(|index| &self.graph[index])
            .collect()
    }
    ...
    pub fn filter_reachable_from(&self, entry_module: &str) -> Result<DependencyGraph> {
        let entry_index = self.node_indices.get(entry_module)
        ...
        while let Some(current_index) = stack.pop() {
            if visited.insert(current_index) {
                ...
                let incoming_count = self.graph.neighbors_directed(current_index, petgraph::Direction::Incoming).count();
                println!("DEBUG: Module {} has {} incoming edges (dependencies)", current_module, incoming_count);
                
                for neighbor_index in self.graph.neighbors_directed(current_index, petgraph::Direction::Incoming) {
                    let neighbor_module = &self.graph[neighbor_index].name;
                    println!("DEBUG: Found dependency: {} -> {}", neighbor_module, current_module);
        ...
        for &from_index in &visited {
            for to_index in self.graph.neighbors_directed(from_index, petgraph::Direction::Incoming) {
