...
function getPlatformPackageName() {
  ...
  return `@serpen/${pkgPlatform}-${pkgArch}${suffix}`;
}
...
function main() {
  try {
    ...
    const binName = process.platform === 'win32' ? 'serpen.exe' : 'serpen';
    ...
    try {
      ...
      console.error(`Error: Could not find Serpen binary for your platform (${pkgName}).`);
      console.error('');
      ...
      console.error('   npm install serpen');
      console.error('   # or');
      console.error('   yarn add serpen');
      console.error('');
    ...
    if (!existsSync(binPath)) {
      ...
      console.error('Please try reinstalling serpen.');
      process.exit(1);
    }
    ...
    try {
      ...
      console.error(`Error executing Serpen binary: ${execError.message}`);
      process.exit(1);
    }
