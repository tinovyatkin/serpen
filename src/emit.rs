...
impl CodeEmitter {
        ...
        for line in source.lines() {
            ...
            if trimmed.starts_with("from ") && trimmed.contains(" import ") {
                ...
                for module in modules {
                    ...
                    match self.resolver.classify_import(module_name) {
                        ImportType::ThirdParty | ImportType::StandardLibrary => {
                            // Keep this module (with its potential alias)
                ...
                // Only add the line if there are non-first-party modules to import
                if !filtered_modules.is_empty() {
                    let filtered_line = format!("import {}", filtered_modules.join(", "));
                    output_lines.push(filtered_line);
                }
        ...
        for stmt in statements {
            match stmt {
                ...
                Stmt::ImportFrom(import_from_stmt) => {
                    // Reconstruct import-from without relative level
                    let module_part = import_from_stmt
                        .module
                        .as_ref()
                        .map(|m| m.as_str())
                        .unwrap_or("");
                    let import_names = import_from_stmt
                        .names
                        .iter()
                        .map(|name| {
    ...
    fn statement_to_string(&self, stmt: &Stmt) -> Result<String> {
        ...
        match stmt {
            ...
            Stmt::ImportFrom(import_from_stmt) => {
                // Reconstruct module part (ignore relative level dots)
                let module_part = import_from_stmt
                    .module
                    .as_ref()
                    .map(|m| m.as_str())
